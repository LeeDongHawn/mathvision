...
void PolygonDemo::refreshWindow()
{
	...
	if (m_data_ready)
	{
		...
		if (m_param.gauss_newton) {
			std::vector<cv::Point2d> point13, point14;
			int iter = 0;
			bool ok = gauss_newton(m_data_pts,point13,point14,iter);
			
			if (ok){
				
				for (int i = 0; i < iter-1; i++) {
					//blue : first sine graph
					line(frame, point13[i], point13[i+1], Scalar(255, 0, 0), 1);
					//red : last sine graph
					line(frame, point14[i], point14[i + 1], Scalar(0, 0, 255), 1);
				}
			}
...
bool PolygonDemo::gauss_newton(const std::vector<cv::Point>& pts, std::vector<cv::Point2d>& point13, std::vector<cv::Point2d>& point14, int& iteration)
{
	...
	float B = 0.01;
	float C = 2.0;
	//printf("y = %d*sin(%.2f*x+%.2f)+%d", A, B, C, D);
	
	//First sin graph
	for (int k = 0; k < n; k++) {
		point13.push_back(cv::Point2d(pts[k].x, A*sin(B*pts[k].x + C) + D));
	}
	iteration = n;

	for (int z = 0; z < 100; z++) {
		//r(p) = y1 - A*sin(B*x1+C)+D
		Mat rp = Mat::zeros(n, 1, CV_32FC1);
		//Jr(p) = 
		Mat Jrp = Mat::zeros(n, 2, CV_32FC1);
		Mat p = Mat::ones(2, 1, CV_32FC1);
		// setting p
		p.at<float>(0, 0) = B;
		p.at<float>(1, 0) = C;
		for (int i = 0; i < pts.size(); i++)
		{
			rp.at<float>(i, 0) = pts[i].y - (A*sin(B * pts[i].x + C) + D);
			Jrp.at<float>(i, 0) = A * pts[i].x * cos(B*pts[i].x + C);
			Jrp.at<float>(i, 1) = A * cos(B*pts[i].x + C);
		}
		//print(rp);
		//printf("\n");
		//print(Jrp);
		...
		//print(trans_Jrp*Jrp);
		Mat out = Mat::ones(2, 2, CV_32FC1);
		out = (trans_Jrp*Jrp).inv();
		//printf("\n");
		//print(out);
		Mat result;
		
		result = p - out*trans_Jrp*rp;
		printf("\n");
		print(result);
		//update parameter B,C
		B = result.at<float>(0, 0);
		C = result.at<float>(1, 0);
	...
	//Last sin graph
	for (int z = 0; z < n; z++) {
		point14.push_back(cv::Point2d(pts[z].x, -1*A*sin(B*pts[z].x + C) + D));
	}
	print(point14);
	return 1;
}
