...
public:
	...
	bool gauss_newton(const std::vector<cv::Point>& pts, std::vector<cv::Point2d>& point13, std::vector<cv::Point2d>& point14, int& iteration);

	bool fit_robust(const std::vector<cv::Point>& pts, std::vector<cv::Point2d>& point5, std::vector<cv::Point2d>& point6, int& iteration);
	bool fit_ransac(const std::vector<cv::Point>& pts, cv::Point2d& point9, cv::Point2d& point10, int& iteration);
	bool fit_ransac2(const std::vector<cv::Point>& pts, std::vector<cv::Point2d>& point11, std::vector<cv::Point2d>& point12, int& iteration);
